...
from preprocessing.audio.time_stretch_processor import TimeStretchProcessorFactor1_1, TimeStretchProcessorFactor0_9

from .dataloader.dataset_loader import AbstractDatasetLoader
from .dataloader.librosa_file_loader import FileLoader
...
from preprocessing.audio.pitch_shift_processor import PitchShiftProcessor1SemitonesDown, PitchShiftProcessor1SemitonesUp
...
class PreprocessorModule(Enum):
    ...
    TIME_STRETCH_1_1:            str = TimeStretchProcessorFactor1_1.__name__
    TIME_STRETCH_0_9:            str = TimeStretchProcessorFactor0_9.__name__
...
class ModularPreprocessor():
    def __init__(
        ...
        self._load_augmented = load_augmented
        ...
        for (key, df) in split.items():
                ...
                for index, chunk in enumerate(chunks):
                    if not self._load_augmented:
                        ...
                        #augmented_data = splitter.process(augmented_data)
                        data = splitter.process(data)

                        #np.savez(f"{self._dataset_path}/augmented/arr_{key}_{str(index)}", data)
                        #if key == "training":
                        #    np.savez(f"{self._dataset_path}/augmented/arr_{key}_augmented_{str(index)}", augmented_data)
                    else:
                        data = np.load(f"{self._dataset_path}/augmented/arr_{key}_{str(index)}.npz", allow_pickle=True)
                        data = data["arr_0"]
                        if key == "training":
                            augmented_data = np.load(f"{self._dataset_path}/augmented/arr_{key}_augmented_{str(index)}.npz", allow_pickle=True)
                            augmented_data = augmented_data["arr_0"]

                    #if key == "training":
                    #    data = list(data) + list(augmented_data)
